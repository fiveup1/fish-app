    ...
    try {
      ...
      const { data: fish, error: insertErr } = await supabase
        ...
        .insert({
          ...
          category,
          ...fields,
          sashimi_grade: fields.sashimi_grade === true || fields.sashimi_grade === 'true' || fields.sashimi_grade === '是',
          aging_days: fields.aging_days ? parseInt(fields.aging_days) : null,
      ...
      let photoUrls = []
      if (photos.length > 0) {
        photoUrls = await uploadPhotos(fish.id)
        await supabase.from('fishes').update({ photos: photoUrls }).eq('id', fish.id)
      }
    ...
    <div style={{
      ...
      <div style={{ padding: '16px 16px 120px' }}>
        ...
        {aiDone && (
          <section style={{
            marginBottom: 20,
            animation: 'bubbleUp 0.4s var(--ease-ocean)',
          }}>
            <div style={{
                ...
                {key === 'sashimi_grade' ? (
                  <div style={{ display: 'flex', gap: 8 }}>
                    {['是', '否', '視情況'].map(opt => (
                      <button
                        key={opt}
                        onClick={() => setFields(f => ({ ...f, sashimi_grade: opt === '是' }))}
                        style={{
                          padding: '6px 14px',
                          borderRadius: 8,
                          fontSize: 13,
                          background: (fields.sashimi_grade === true && opt === '是') || (fields.sashimi_grade === false && opt === '否')
                            ? 'rgba(0, 229, 255, 0.15)' : 'var(--bg-card)',
                          color: (fields.sashimi_grade === true && opt === '是') || (fields.sashimi_grade === false && opt === '否')
                            ? 'var(--accent-biolum)' : 'var(--text-muted)',
        ...
        <section style={{ marginBottom: 24 }}>
          <label style={{ ...labelStyle, display: 'flex', justifyContent: 'space-between' }}>
            <span>照片 <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>({photos.length}/10)</span></span>
            {photos.length < 10 && (
              <button
